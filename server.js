...
pool.connect((err, client, release) => {
  if (err) {
    ...
    console.log(' РЈСЃРїРµС€РЅРѕРµ РїРѕРґРєР»СЋС‡РµРЅРёРµ Рє PostgreSQL');
    release();
  }
});
...
app.get('/api/subjects/:class', async (req, res) => {
  try {
    ...
    console.error('РћС€РёР±РєР° РїРѕР»СѓС‡РµРЅРёСЏ РїСЂРµРґРјРµС‚РѕРІ:', error);
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    ...
    console.error('РћС€РёР±РєР° РїРѕР»СѓС‡РµРЅРёСЏ Р»РёРґРµСЂР±РѕСЂРґР°:', error);
    res.status(500).json({ error: 'Р’РЅСѓС‚СЂРµРЅРЅСЏСЏ РѕС€РёР±РєР° СЃРµСЂРІРµСЂР°' });
  }
});
...
app.get('/api/search', async (req, res) => {
  try {
    ...
    const subjectsResult = await pool.query(
      `SELECT name as title, description, 'РџСЂРµРґРјРµС‚' as type, 'fas fa-book' as icon
       FROM subjects 
    ...
    const results = [
      ...subjectsResult.rows,
      {
        ...
        description: "Р РµР№С‚РёРЅРі Р»СѓС‡С€РёС… СѓС‡РµРЅРёРєРѕРІ",
        type: "Р РµР№С‚РёРЅРі",
...
app.post('/api/progress', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'РџСЂРѕРіСЂРµСЃСЃ РѕР±РЅРѕРІР»РµРЅ' });
  } catch (error) {
    console.error('РћС€РёР±РєР° РѕР±РЅРѕРІР»РµРЅРёСЏ РїСЂРѕРіСЂРµСЃСЃР°:', error);
    res.status(500).json({ error: 'Р’РЅСѓС‚СЂРµРЅРЅСЏСЏ РѕС€РёР±РєР° СЃРµСЂРІРµСЂР°' });
  }
});
...
app.post('/api/init-demo', async (req, res) => {
  try {
    ...
    await pool.query(`
      ...
      ('elena_v', 'Р•Р»РµРЅР° Р’.', 9),
      ('vasya', 'Р’Р°СЃСЏ', 8),
      ('evgeniy', 'Р•РІРіРµРЅРёР№', 7)
      ON CONFLICT (username) DO NOTHING;
      
      INSERT INTO subjects (name, description, class_number) VALUES
      ('Р¤РёР·РёРєР°', 'РР·СѓС‡РµРЅРёРµ Р·Р°РєРѕРЅРѕРІ РїСЂРёСЂРѕРґС‹', 7),
      ('Р¤РёР·РёРєР°', 'РР·СѓС‡РµРЅРёРµ Р·Р°РєРѕРЅРѕРІ РїСЂРёСЂРѕРґС‹', 8),
      ('Р¤РёР·РёРєР°', 'РР·СѓС‡РµРЅРёРµ Р·Р°РєРѕРЅРѕРІ РїСЂРёСЂРѕРґС‹', 9)
      ...
      ('РћСЃРµРЅРЅРёР№ С‚СѓСЂРЅРёСЂ', 'РўСѓСЂРЅРёСЂ РїРѕ С„РёР·РёРєРµ', 520, 1000),
      ('Р’РµСЃРµРЅРЅСЏСЏ РѕР»РёРјРїРёР°РґР°', 'РЁРєРѕР»СЊРЅР°СЏ РѕР»РёРјРїРёР°РґР°', 720, 1000)
      ON CONFLICT DO NOTHING;
    `);

    res.json({ success: true, message: 'Р”РµРјРѕ РґР°РЅРЅС‹Рµ РёРЅРёС†РёР°Р»РёР·РёСЂРѕРІР°РЅС‹' });
  } catch (error) {
    console.error('РћС€РёР±РєР° РёРЅРёС†РёР°Р»РёР·Р°С†РёРё РґР°РЅРЅС‹С…:', error);
    res.status(500).json({ error: 'Р’РЅСѓС‚СЂРµРЅРЅСЏСЏ РѕС€РёР±РєР° СЃРµСЂРІРµСЂР°' });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(` РћС‚РєСЂРѕР№С‚Рµ РІ Р±СЂР°СѓР·РµСЂРµ: http://localhost:${PORT}`);
});